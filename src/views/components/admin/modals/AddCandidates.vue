<template>
  <div class="modal-overlay-2 flex">
    ...
    >
      ...
      <div v-if="positionStore.candidates[0]">
        <div class="overflow-y-scroll h-52">
          <div class="grid p-4 grid-cols-4">
            ...
            >
              <div
                :class="{ 'bg-sky-600': candidate.isSelected }"
                class="h-16 rounded-full"
              >
                <img
                  class="w-full p-1 h-16 rounded-full"
                  :src="candidate.image"
                />
              </div>
      ...
      <div class="p-4 h-52" v-else>
        No more available candidates <router-link :to="{ name: 'candidates' }"
    ><span class="text-cyan-500 hover:underline">add</span></router-link
  > here
      </div>

      <button
        @click="positionStore.handleInsertCandidates"
        class="btn btn-green"
      >
        Submit
      </button>
...
export default {
  setup() {
    ...
    const computedCandidates = computed(() => {
      ...
      return positionStore.candidates.filter((item) =>
